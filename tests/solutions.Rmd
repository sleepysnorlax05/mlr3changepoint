...
As visualised above, both algorithms identify nearly identical segmentation breaks for the `Lai2005fig4` sequence. Although **PELT** is mathematically guaranteed to find the global optimum and **Binary Segmentation** only performs an approximate greedy search, the extremely high signal-to-noise ratio in this genomic data ensures that the simpler heuristic correctly reaches the same functional conclusion.
...
To fit these mathematical bounds, I encapsulated the fundamental `penaltyLearning::IntervalRegressionCV` function inside my custom learner blueprint, generating `LearnerIntRegrCV`. This newly structured framework intrinsically interprets the dual interval margins, running cross-validation algorithms directly on the internal matrices to compute a functional regression line maximising the physical margin between valid and invalid penalty bounds.
...
The resulting benchmark confirms that the cross-validation fold mechanism smoothly navigates the entire matrix grid in only a handful of seconds, proving the underlying scale limitations hold steady.
...
1. **Constant Baseline**: An arbitrary, bluntly fixed log-penalty statically defaulting at 0 unconditionally.
2. **BIC Baseline**: The standard $\log(n)$ parameter vector, cleanly imitating the mathematically inflexible heuristics demonstrated above.
...
Graphing out the comparative ROC results:

![](plots/hard/roc_plot.png)

As demonstrated throughout the resulting ROC graph and strictly supported by the final Area Under the Curve (AUC) statistics, the supervised prediction technique definitively proves superior. The globally static **BIC** metric objectively falters, generating an AUC essentially identical to the arbitrary **Constant** baseline. Its inherent inflexibility directly accounts for severe failure rates when forced to segment highly inconsistent biological arrays.
...
**Description:**
An R6 class that encapsulates unsupervised time-series or sequential data. It handles internal data subsetting, feature identification, and spatial/positional tracking.
...
When invoked, the task locks the dataset into a `DataBackend`. It designates the `target` as the primary feature matrix for the learner while preserving the `position_col` safely outside the modelling features, enabling it to be mapped back during plotting.
...
**Description:**
An R6 class bridging external changepoint algorithms with the `mlr3` training and prediction steps. Rather than hardcoding a single package, it accepts custom "engines" to execute algorithm-specific logic.
...
**Operations:**
*   **`$train(task)`**: Subsets the task data based on operational `row_ids` and passes it to the `train_engine`. Returns an instantiated model containing internal parameter configurations.
*   **`$predict(task)`**: Evaluates the fitted state against a task. For unsupervised algorithms, this directly outputs the identified breakpoint indices to the prediction object.

---

### 3. `PredictionCpt` (Class)

**Description:**
An R6 class that standardises the varied outputs of different algorithmic packages (e.g., lists, S4 objects, raw vectors) into a strict `mlr3` framework.

**Outputs / Return Mechanics:**
Once `$predict()` is termed, a `PredictionCpt` object is returned:
*   **`$cpts`** (*integer vector*): The exact structural breaks (changepoint indices) identified in the sequence. 
*   **`$data`** (*data.table*): The matching sequence snippet, alongside calculated structural metrics like segment means/variances.

---

### 4. Visualisation: `autoplot.PredictionCpt` (Method)

**Description:**
An S3 method extending `ggplot2` and `mlr3viz` to natively render changepoint results parsed from the prediction objects.

**Usage:**
```r
autoplot(prediction_object, task, type = "trace")
```
**Supported Types:**
